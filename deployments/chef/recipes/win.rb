...
windows_zipfile node['signalfx_agent']['install_dir'] do
  ...
  action :unzip
  only_if {not ::File.exists?(node['signalfx_agent']['version_file']) or ::File.readlines(node['signalfx_agent']['version_file']).first.strip != node['signalfx_agent']['package_version']}
