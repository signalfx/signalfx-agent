...
| `observers` | no | [list of object](#observers) | A list of observers to use (see observer config) |
| `monitors` | no | [list of object](#monitors) | A list of monitors to use (see monitor config) |
| `writer` | no | [object](#writer) | Configuration of the datapoint/event writer |
| `logging` | no | [object](#logging) | Log configuration |
| `collectd` | no | [object](#collectd) | Configuration of the managed collectd subprocess |
| `metricsToInclude` | no | [list of object](#metricstoinclude) | A list of metric filters that will whitelist/include metrics.  These filters take priority over the filters specified in `metricsToExclude`. |
| `metricsToExclude` | no | [list of object](#metricstoexclude) | A list of metric filters |
| `propertiesToExclude` | no | [list of object](#propertiestoexclude) | A list of properties filters |
...
| `configSources` | no | [object](#configsources) | Configuration of remote config stores |
...
| `solo` | no | bool | If one or more configurations have this set to true, only those configurations will be considered considered. This setting can be useful (**default:** `false`) |
| `metricsToExclude` | no | [list of object](#metricstoexclude) | A list of metric filters |
...
| `file` | no | [object](#file) | Configuration for other file sources |
| `zookeeper` | no | [object](#zookeeper) | Configuration for a Zookeeper remote config source |
| `etcd2` | no | [object](#etcd2) | Configuration for an Etcd 2 remote config source |
| `consul` | no | [object](#consul) | Configuration for a Consul remote config source |
| `vault` | no | [object](#vault) | Configuration for a Hashicorp Vault remote config source |
