...
def test_kafka_in_k8s(agent_image, minikube, k8s_observer, k8s_test_timeout, k8s_namespace):
    monitors = [
        {"type": "collectd/kafka",
         "discoveryRule": 'container_image =~ "kafka" && private_port == 7302 && kubernetes_namespace == "%s"' % k8s_namespace,
         "serviceURL": 'service:jmx:rmi:///jndi/rmi://{{.Host}}:{{.Port}}/jmxrmi',
         "username": "testuser", "password": "testing123"},
    ]
    yamls = [os.path.join(os.path.dirname(os.path.realpath(__file__)), "kafka-k8s.yaml")]
    run_k8s_monitors_test(
        agent_image,
        minikube,
        monitors,
