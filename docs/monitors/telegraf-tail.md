...
| `telegrafParser` | no | `object (see below)` | telegrafParser is a nested object that defines configurations for a Telegraf parser. Please refer to the Telegraf for more information on Telegraf parsers. |
...
| `JSONTagKeys` | no | `list of string` | A list of tag names to fetch from JSON data. (`json` only) |
| `JSONStringFields` | no | `list of string` | A list of fields in JSON to extract and use as string fields. (json only) |
...
| `templates` | no | `list of string` | A list of templates for Graphite data. (`graphite` only). |
...
| `collectdTypesDB` | no | `list of string` | A list of paths to collectd TypesDB files. (`collectd` only) |
...
| `dropwizardTagPathsMap` | no | `map of string` | A map of gjson tag names and gjson paths used to extract tag values from the JSON document. This is only used if `dropwizardTagsPath` is not specified. (`dropwizard` only) |
| `grokPatterns` | no | `list of string` | A list of patterns to match. (`grok` only) |
| `grokNamedPatterns` | no | `list of string` | A list of named grok patterns to match.  (`grok` only) |
| `grokCustomPatterns` | no | `string` | Custom grok patterns. (`grok` only) |
| `grokCustomPatternFiles` | no | `list of string` | List of paths to custom grok pattern files. (`grok` only) |
