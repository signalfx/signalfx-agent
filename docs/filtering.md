...
`dimensionValue`, `propertyName`, `propertyValues`. 

Sometimes it is easier to whitelist the properties you want to allow through.
You can do this by prefixing any config value with `!`. 
...
```yaml
  ...
  propertiesToExclude:
    ...
    # Only sync 'pod-template-hash' on 'kubernetes_pod_uid'
    - dimensionName: kubernetes_pod_uid
      propertyName: !pod-template-hash
