...
// GAUGE(win_services.state): The current state of the service: 1 (stopped),
// 2 (start pending), 3 (stop pending), 4 (running), 5 (continue pending),
// 6 (pause pending), or 7 (paused)

// GAUGE(win_services.startup_mode): The startup mode configured for the
// service: 0 (boot start), 1 (system start), 2 (auto start), 3 (demand start),
// or 4 (disabled)
