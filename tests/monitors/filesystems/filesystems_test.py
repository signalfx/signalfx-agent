...
def test_filesystems_extra_metrics():
    an_extra_metric = "percent_inodes.used"
    expected_metrics = METADATA.included_metrics | {an_extra_metric}
    agent_config = f"""
                    procPath: /proc
                    monitors:
                    - type: filesystems
...
def test_filesystems_extra_metrics_by_group_flag():
    expected_metrics = METADATA.included_metrics | {"percent_bytes.free", "percent_bytes.used"}
    a_group_flag = "includeLogical: true"
    agent_config = f"""
                    procPath: /proc
                    monitors:
                    - type: filesystems
