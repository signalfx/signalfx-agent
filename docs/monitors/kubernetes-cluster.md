...
| `creation_timestamp` | `kubernetes_node_uid/kubernetes_pod_uid/kubernetes_uid` | CreationTimestamp is a timestamp representing the server time when this object was created and is in UTC. This property is synced onto `kubernetes_node_uid`, `kubernetes_pod_uid` or `kubernetes_uid` depending on whether it's a node or Kubernetes workload. |
