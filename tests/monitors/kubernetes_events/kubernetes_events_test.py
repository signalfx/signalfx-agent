...
from tests.helpers.util import wait_for, ensure_always
...
def test_k8s_events_with_whitelist(agent_image, minikube, k8s_observer, k8s_test_timeout, k8s_namespace):
    ...
    with run_k8s_with_agent(
            ...
            namespace=k8s_namespace) as [backend, agent]:
...
@pytest.mark.k8s
@pytest.mark.kubernetes
def test_k8s_events_without_whitelist(agent_image, minikube, k8s_observer, k8s_test_timeout, k8s_namespace):
    monitors = [
        {"type": "kubernetes-events",
         "kubernetesAPI": {"authType": "serviceAccount"}},
    ]
