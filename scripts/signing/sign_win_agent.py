...
def getargs():
    parser = argparse.ArgumentParser(
        formatter_class=argparse.RawDescriptionHelpFormatter, description="Sign the windows agent."
    )
    parser.add_argument("path", type=str, metavar="PATH", help="Path to the windows agent bundle zip/msi file.")
    parser.add_argument(
        "--timeout",
        type=int,
...
def main():
    ...
    assert ext in (".msi", ".zip"), f"Unknown file type {args.path}"
    ...
    if ext == ".zip":
        ...
        dest = os.path.join(signed_dir, os.path.basename(args.path))
        sign_file(
            args.path, dest, "WIN", args.chaperone_token, args.staging_user, args.staging_token, args.timeout
        )
