...
def test_haproxy_in_k8s(agent_image, minikube, k8s_observer, k8s_test_timeout, k8s_namespace):
    monitors = [
        {"type": "collectd/haproxy",
         "discoveryRule": 'container_image =~ "haproxy" && private_port == 9000 && kubernetes_namespace == "%s"' % k8s_namespace,
         "enhancedMetrics": True},
    ]
    yamls = [os.path.join(os.path.dirname(os.path.realpath(__file__)), "haproxy-k8s.yaml")]
    run_k8s_monitors_test(
        agent_image,
        minikube,
        monitors,
