...
values. The default configured period is 10 minutes which works across all measurements and gives a reasonable
...
| `period` | no | `string` | Period the duration in ISO 8601 notation that specifies how far back in the past to retrieve measurements from Atlas. (**default:** `PT10M`) |
