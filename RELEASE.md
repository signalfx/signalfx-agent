...
1. Ensure you are on the Splunk network and can access the
   `splunk.jfrogi.io Artifactory Token`, `repo.splunk.com Artifactory Token`,
   and `srv-signalfx-agent Chaperone Token` service account secrets in
   [Thycotic](https://secret.splunk.com).
...
1. Run the release script:
   ...
   Using the service account tokens from Thycotic and your personal Github
   token created earlier in the Setup section.
...
1. You must be on the Splunk network and have access to the
   `repo.splunk.com Artifactory Token` and `srv-signalfx-agent Chaperone Token`
   service account secrets in [Thycotic](https://secret.splunk.com) to sign the agent
   executable.
