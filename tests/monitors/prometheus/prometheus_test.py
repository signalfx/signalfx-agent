...
def test_prometheus_in_k8s(agent_image, minikube, k8s_observer, k8s_test_timeout, k8s_namespace):
    monitors = [
        {"type": "prometheus-exporter",
         "discoveryRule": 'container_image =~ "prometheus" && private_port == 9090 && kubernetes_namespace == "%s"' % k8s_namespace,
         "useHTTPS": False,
         "skipVerify": True,
         "metricPath": "/metrics"},
    ]
    yamls = [os.path.join(os.path.dirname(os.path.realpath(__file__)), "prometheus-k8s.yaml")]
