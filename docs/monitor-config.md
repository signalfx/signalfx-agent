...
Monitors gather metrics from the host and from running applications.  They are configured in a list called `monitors` in the [main agent config file](./config-schema.md). These are all of the monitors included in the agent, along with their possible configuration options:
