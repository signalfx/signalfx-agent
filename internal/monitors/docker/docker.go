...
// Validate docker-container-stats monitor config
func (c *Config) Validate() error {
	for _, group := range c.ExtraGroups {
		if _, exists := groupSet[group]; !exists {
			return fmt.Errorf("%s is not a valid group name", group)
		}
	}
...
func (c *Config) GetExtraMetrics() []string {
	...
	logger := logrus.WithFields(logrus.Fields{"monitorType": "GetExtraMetrics"})
	logger.WithFields(logrus.Fields{"extraMetrics":extraMetrics}).Debug("Extra metrics")
