...
docker run --rm -v ${SRC_ROOT}/${AGENT_IMAGE_ROOT}:/opt/build ${BUILDER_IMAGE_NAME}:${TAG} \
    bash -c "cp /usr/local/lib/collectd/{libcollectd,java,mysql,nginx,python,aggregation}.so /usr/local/lib/collectd/generic-jmx.jar /opt/neomock/neomock /opt/go/bin/agent /opt/build"
