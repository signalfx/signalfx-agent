...
def test_health_checker_in_k8s(agent_image, minikube, k8s_observer, k8s_test_timeout, k8s_namespace):
    monitors = [
        {"type": "collectd/health-checker",
         "discoveryRule": 'container_image =~ "json-server" && private_port == 80 && kubernetes_namespace == "%s"' % k8s_namespace,
         "url": 'http://{{.Host}}:{{.Port}}/health',
         "jsonKey": "status",
         "jsonVal": "ok"},
    ]
    yamls = [os.path.join(os.path.dirname(os.path.realpath(__file__)), "health-checker-k8s.yaml")]
