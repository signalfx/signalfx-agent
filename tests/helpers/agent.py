...
class Agent:
    ...
    def current_status_text(self):
        status_proc = subprocess.Popen(
            [AGENT_BIN, "status", "-config", self.config_path], stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
    ...
    @classmethod
    @contextmanager
    def run(cls, init_config, debug=True, backend_options=None, host=None, extra_env=None, profiling=False):
        if host is None:
